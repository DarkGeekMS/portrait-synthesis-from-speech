...
from pybert.train.losses import BCEWithLogLoss, CrossEntropyWithLogLoss, MSELoss
...
from pybert.train.metrics import AUC, AccuracyThresh, MultiLabelReport, MultiLabelAccuracy
...
def run_train(args):
    ...
    trainer = Trainer(args= args,model=model,logger=logger,criterion=MSELoss(),optimizer=optimizer,
                      scheduler=scheduler,early_stopping=None,training_monitor=train_monitor,
                      model_checkpoint=model_checkpoint,
                      batch_metrics=[MultiLabelAccuracy()],
                      epoch_metrics=[])
...
def run_test(args):
    ...
    data.save_test_results(label_list, sentences, results, path_to_save)
